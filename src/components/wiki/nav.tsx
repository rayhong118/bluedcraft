...
const navStyles: Partial<INavStyles> = { root: { width: '100%', position: "relative", marginLeft: "auto" } };
...
export const WikiNav: React.FC<ListOfArticlesProps> = ({
    ...
    function searchArticle(event?: React.ChangeEvent<HTMLInputElement>, key?: string) {
        if (key) {
            let linkGroups: INavLink[] = [];
            navLinkGroups.forEach((group) => {
                (group.links.forEach((link) => {
                    if (link.name.indexOf(key) >= 0)
    ...
    return (
        <div>
            <SearchBox placeholder="Search" styles={navStyles} onChange={searchArticle} iconProps={searchIcon} underlined={true} />
            <Nav styles={navStyles} ariaLabel="Nav" groups={linkGroups} onLinkClick={(e, event) => { e?.preventDefault(); if (event) navigate(event.url) }} />
        </div>
    );
};